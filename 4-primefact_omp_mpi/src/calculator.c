...
void verificar_factorizacion(array_entrada_t* entradas, uint64_t unit) {
  if (entradas->elements[unit].category == COMPOUND) {
     // factorizar_compuesto(entrada)
       factorizar_compuesto(&entradas->elements[unit]);
  }
}  // end procedure
...
int calculate_start(int process_number, int end, int process_count
  , int begin) {
  const int range = end - begin;
  int start = begin + process_number * (range / process_count)
    + minimo(process_number, range % process_count);
  return start  % 2 == 0 ? start : start + 1;
}

int calculate_finish(int process_number, int end, int process_count
  , int begin) {
  int finish = calculate_start(process_number + 1, end, process_count, begin);
  return finish  % 2 == 0 ? finish : finish + 1;
}
...
uint64_t* randomize_process_array(uint64_t* process_array, int process_count) {
  // pide memoria
  process_array = (uint64_t*) calloc(process_count, sizeof(uint64_t));
  process_array = fill_process_array(process_array, process_count);

  // randomiza sus valores
  srand(time(NULL));
  if (process_count > 1) {
    int index;
    for (index = 0; index < process_count - 1; index++) {
      int aux = index + rand() / (RAND_MAX / (process_count - index) + 1);  // NOLINT
      int temp = process_array[aux];
      process_array[aux] = process_array[index];
      process_array[index] = temp;
    }
  }
  return process_array;
}
