 ...
 */
int64_t array_entrada_increase_capacity(array_entrada_t* array);

int64_t array_entrada_init(array_entrada_t* array) {
  assert(array);
  array->capacity = 0;
  array->count = 0;
  array->elements = NULL;
...
void array_entrada_destroy(array_entrada_t* array) {
  assert(array);
...
int64_t array_entrada_append(array_entrada_t* array, entrada_t* element) {
  assert(array);
  if (array->count == array->capacity) {
    if (array_entrada_increase_capacity(array) != EXIT_SUCCESS) {
      return EXIT_FAILURE;
    }
  }
